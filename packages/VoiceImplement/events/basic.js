...
module.exports =
{
	"playerJoin" : player =>
	{
		...
		// ïðèñâîèì èãðîêó ñëó÷àéíóþ ìîäåëü
		...
		// çàñïàâíèì â îäíîé èç òî÷åê, óêàçàííûõ â game.spawns
		player.spawn(game.spawns[Math.floor(Math.random() * game.spawns.length)]);

		player.veh = null;
		player.kills = 0;
		player.outputChatBox("Voice Chat is enabled global");
	},
	...
	{
		// ìû ìîæåì èñïîëüçîâàòü html-òåãè â ÷àòå, òàê êàê îí ñäåëàí íà HTML (cef),
		// èñïîëüçóåì òåã âûäåëåíèÿ äëÿ âûäåëåíèÿ êîëè÷åñòâà óáèéñòâ ó êàæäîãî
		const str = player.name + "<b>(" + player.kills + ")</b>: " + striptags(text, '<b><s><u><strong>');
		
		// îòïðàâèì ñîîáùåíèå âñåì èãðîêàì
		mp.players.forEach(_player => { if(_player.dimension == player.dimension) _player.outputChatBox(str); });
	}
};